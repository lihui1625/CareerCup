...
public class Floyed {
  ...
  public static void main(String[] args) {
    int[][] linkedMatrix = new int[][] { { 0, 50, UNLIMITED, 80, UNLIMITED },
    ...
    for (int i = 0; i < M; i++) {
      for (int j = 0; j < M; j++) {
        System.out
            .print((connectTable[i][j] == UNLIMITED ? "-" : connectTable[i][j])
                + "\t");
      }
      System.out.println();
    }
    ...
    for (int i = 0; i < M2; i++) {
      for (int j = 0; j < M2; j++) {
        System.out
            .print((connectTable2[i][j] == UNLIMITED ? "-" : connectTable[i][j])
                + "\t");
      }
      System.out.println();
    }
